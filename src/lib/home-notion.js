...
// Helper to transform a Notion page into a Section object
async function transformNotionPageToSection(page, index) {
...
async function fetchHomeContentFromNotion() {
    ...
    try {
        ...
        // Map over pages
        const sections = await Promise.all(response.results.map((page, index) => transformNotionPageToSection(page, index)));

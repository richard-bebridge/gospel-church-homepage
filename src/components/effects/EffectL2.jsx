...
const Section = ({ title, children, index, setActiveSection }) => {
    ...
    return (
        <div ref={ref} className="h-screen snap-start flex flex-col justify-center px-8 lg:px-24 border-b border-gray-200 last:border-0 py-24 relative z-0">
            <motion.div
...
const EffectL2 = ({ setActiveSection }) => {
    const BLUR_ZONE_RATIO = 0.30; // 30%

    return (
        <div className="w-full lg:w-1/2 bg-[#F4F3EF] relative">
            {/* Content wrapper */}
            <div
                className="relative"
            >
            ...
            {/* Blur overlays - positioned absolutely to create the blur effect */}
            <div
                className="absolute top-0 left-0 w-full pointer-events-none z-20"
                style={{
                    height: `${BLUR_ZONE_RATIO * 100}vh`,
